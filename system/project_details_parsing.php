...
function MailParse( $name, &$html, $show_desc) {
  ...
  if( file_exists($group_file) ) {
    ...
    for ( $loop = 1; $loop < $group_count; $loop+=2) {
	  ...
	  $html .= "<blockquote> <a onclick=\"switchMenu('$mail_name');\" title=\"desc\">Desc</a> $mail_html $mailarch_html </blockquote> <div id=\"$mail_name\" class=\"switchcontent\"> <p> $description </p></div>";
	}
  }          
}
